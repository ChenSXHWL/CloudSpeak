...
#if CONFIG_P2P_PPT
/*StreamType*/
typedef enum sType_e{
	StreamType_Audio,	//AudioStream
	StreamType_Video	//VideoStream
} sType;
#endif
...
#if CONFIG_P2P_PPT
ORTP_PUBLIC int p2p_session_created(const char * szUID);
ORTP_PUBLIC int p2p_session_destroy(void);
ORTP_PUBLIC int p2p_session_open(const char * szDevID);
ORTP_PUBLIC int p2p_session_close(void);

ORTP_PUBLIC int p2p_session_event(unsigned int *uSessType);
ORTP_PUBLIC int p2p_session_connect(void);
#endif
ORTP_PUBLIC int
