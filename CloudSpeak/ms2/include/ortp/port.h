...
#define CONFIG_DBG_MALLOC			0


ORTP_PUBLIC void* ortpbuf_malloc(size_t sz
							#if CONFIG_DBG_MALLOC
								,const char *file
								,int lineno
							#endif
								);
ORTP_PUBLIC void ortpbuf_free(void *ptr);
ORTP_PUBLIC void* ortpbuf_realloc(void *ptr, size_t sz
							#if CONFIG_DBG_MALLOC
								,const char *file
								,int lineno
							#endif
								);
ORTP_PUBLIC void* ortpbuf_malloc0(size_t sz
							#if CONFIG_DBG_MALLOC
								,const char *file
								,int lineno
							#endif
								);
ORTP_PUBLIC char * ortpbuf_strdup(const char *tmp
							#if CONFIG_DBG_MALLOC
								,const char *file
								,int lineno
							#endif
...
ORTP_PUBLIC void ortpbuf_check(void);

#if CONFIG_DBG_MALLOC

#define ortp_malloc(sz)  		ortpbuf_malloc(sz,__FUNCTION__,__LINE__)
#define ortp_malloc0(sz)		ortpbuf_malloc0(sz,__FUNCTION__,__LINE__)
#define ortp_realloc(ptr,sz) 	ortpbuf_realloc(ptr,sz,__FUNCTION__,__LINE__)
#define ortp_strdup(tmp)		ortpbuf_strdup(tmp,__FUNCTION__,__LINE__)
#define ortp_free(ptr) 			ortpbuf_free(ptr)
#define ortp_check()			ortpbuf_check()

/*override the allocator with this method, to be called BEFORE ortp_init()*/
typedef struct _OrtpMemoryFunctions{
	void *(*malloc_fun)(size_t sz,const char *file,int lineno);
	void *(*realloc_fun)(void *ptr, size_t sz,const char *file,int lineno);
	void (*free_fun)(void *ptr);
	void (*check_fun)(void);
}OrtpMemoryFunctions;

#else
#define ortp_malloc(sz)  		ortpbuf_malloc(sz)
#define ortp_malloc0(sz)		ortpbuf_malloc0(sz)
#define ortp_realloc(ptr,sz) 	ortpbuf_realloc(ptr,sz)
#define ortp_strdup(tmp)		ortpbuf_strdup(tmp)
#define ortp_free(ptr) 			ortpbuf_free(ptr)
