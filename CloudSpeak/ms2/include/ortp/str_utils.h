...
#define return_if_fail(expr) if (!(expr)) {ortp_warning("%s:%i- assertion"#expr "failed\n",__FILE__,__LINE__); return;}
#define return_val_if_fail(expr,ret) if (!(expr)) {ortp_warning("%s:%i- assertion" #expr "failed\n",__FILE__,__LINE__); return (ret);}
...
#define allocb(size, pri)  		allocb_test(size, pri,__FUNCTION__,__LINE__)
ORTP_PUBLIC mblk_t *allocb_test(int size, int pri,	const char *file,int lineno);
...
#define freemsg(mp)  test_freemsg(mp,__FUNCTION__,__LINE__)

ORTP_PUBLIC void test_freemsg(mblk_t *mp,	const char *file,int lineno);
