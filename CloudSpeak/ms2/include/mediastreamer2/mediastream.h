...
/* Payload type */
#define RTP_PT_CODEC_PCMU			0
#define RTP_PT_CODEC_PCM			2
#define RTP_PT_CODEC_PCMA			8
#define RTP_PT_CODEC_G729			18
#define RTP_PT_CODEC_SPEEX			36
#define VIDEO_PT_CODEC_H264			102

/* IOTC define */
#define 	IOTC_REINIT_SOCKET			0
#define 	IOTC_SEND_IOCTRL_EXIT		1
#define 	IOTC_CONNECT_STOP_BY_SID	2

#define 	IOTC_NET_MODE_P2P			0
#define 	IOTC_NET_MODE_RLY			1
#define 	IOTC_NET_MODE_LAN			2

//TUTK model CallBack
#define 	TUTK_CB_NULL					0
#define 	TUTK_CB_CALL_RELEASE			1
#define 	TUTK_CB_CALL_VIDEO_RECEIVED		2
#define 	TUTK_CB_RECV_DATA_TIMEOUT		3
 ...
 * @}
**/
typedef void (*cbTutk)(int nEvent);
...
struct _MediaStream {
	...
	RtpSession *session;
	OrtpEvQueue *evq;
#if CONFIG_TUTK_ENABLE
	MSFilter *tutkrecv;//TUTK
	MSFilter *tutksend;
#endif	
	MSFilter *rtprecv;
	MSFilter *rtpsend;
...
#define AUDIO_STREAM_FEATURE_TEST				(1 << 8)
#define AUDIO_STREAM_FEATURE_AGC				(1 << 9)
...
#define DTMF_TYPE_INBAND	1
#define DTMF_TYPE_RFC2833	2
 ...
 *  send a dtmf
 *  */
MS2_PUBLIC int audio_stream_send_dtmf (AudioStream *stream, char dtmf,char dtmfType);
...
MS2_PUBLIC void audio_stream_set_mic_vol(AudioStream *stream,float gain);

MS2_PUBLIC void audio_stream_set_spk_vol(AudioStream *stream,float gain);

MS2_PUBLIC float audio_stream_get_mic_vol(AudioStream *stream);

MS2_PUBLIC float audio_stream_get_spk_vol(AudioStream *stream);

MS2_PUBLIC void check_malloc(void);

//tutk
#if CONFIG_TUTK_ENABLE
MS2_PUBLIC void IOTC_Init(cbTutk  pAppCb);
MS2_PUBLIC void IOTC_Uninit(void);

MS2_PUBLIC int IOTC_LanSearchDevices(struct st_LanSearchInfo *psLanSearchInfo,int nArrayLen);
MS2_PUBLIC int IOTC_RecvIMsg(const char* uid,unsigned int *ioType, char *ioCtrlBuf, int nIOCtrlMaxDataSize,unsigned int nTimeout);
MS2_PUBLIC int IOTC_SendIMsg(const char* uid,const char *msg);
MS2_PUBLIC int IOTC_CallTutkApi(int api,const char* uid);
MS2_PUBLIC int IOTC_GetConnectedNetMode(const char* uid);
MS2_PUBLIC int IOTC_ConnectDevice(const char* uid,const char* account,const char* pwd);
MS2_PUBLIC int IOTC_DisconnectDevice(const char* uid);
MS2_PUBLIC AudioStream * tutk_audio_stream_start(int payload,const char* uid);
MS2_PUBLIC void tutk_audio_stream_stop(AudioStream * stream);

#elif CONFIG_SIP_ENABLE//sip

MS2_PUBLIC AudioStream * sip_audio_stream_start(int local_port,const char *remote_ip, int remote_port,int payload, int jitt_comp);
MS2_PUBLIC void sip_audio_stream_stop(AudioStream * stream);
MS2_PUBLIC bool_t sip_audio_stream_alive(AudioStream * stream);
...
#if CONFIG_TUTK_ENABLE
MS2_PUBLIC VideoStream * tutk_video_stream_start (int payload,const char* uid,unsigned long window_id);
MS2_PUBLIC void tutk_video_stream_stop (VideoStream * stream);

#elif CONFIG_SIP_ENABLE//sip

MS2_PUBLIC VideoStream * sip_video_stream_start (int local_port,const char *remote_ip, int remote_port,int payload, int jitt_comp,unsigned long window_id);
MS2_PUBLIC void sip_video_stream_stop (VideoStream * stream);
...
MS2_PUBLIC void video_stream_shapshot(VideoStream *stream, const char *filename);
