...
- (void)addTipsLabel {
    self.tipsLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMinX(self.scannerView.frame), CGRectGetMaxY(self.scannerView.frame) * kHeightPaddingAspect + self.scannerView.frame.size.width - 80, self.scannerView.frame.size.width, 20)];
    self.tipsLabel.textAlignment = NSTextAlignmentCenter;
    self.tipsLabel.textColor = [UIColor grayColor];
    self.tipsLabel.text =  @"放入框内，自动扫描";
...
- (void)setupAVCaptureComponent:(CGRect)rect {
    ...
    if (error || !self.captureDeviceInput) {
        EZLog(@"error: %@", [error localizedDescription]);
        return ;
    }
