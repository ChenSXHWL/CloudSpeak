...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *oneDayStr = [dateFormatter stringFromDate:selectedDate];
    NSString *anotherDayStr = [dateFormatter stringFromDate:[NSDate date]];
    if (anotherDayStr.intValue>oneDayStr.intValue) {
        [AYProgressHud progressHudShowShortTimeMessage:@"请选择大于或等于今天的日期"];
        return;
    }
    FSCalendarMonthPosition monthPosition = [self.calculator monthPositionForIndexPath:indexPath];
    FSCalendarCell *cell;
...
- (void)deselectDate:(NSDate *)date
{
    ...
    self.allowNumber = self.allowNumber-1;
}
